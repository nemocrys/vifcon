...
class NemoAchseLinWidget(QWidget):
    def __init__(self, sprache, Frame_Anzeige, widget, line_color, config, config_dat, neustart, multilog_aktiv, add_Ablauf_function, nemoAchse, gamepad_aktiv, typ = 'Antrieb', parent=None):
        ...
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ### Anzeige eines betätigen Knopfes:
        ...
        if not type(self.PID_Aktiv) == bool and not self.PID_Aktiv in [0,1]: 
            ...
            self.PID_Aktiv = 0
        ### Gamepad-Button:
        if not self.Button_Link in ['HubS', 'HubT']:
            logger.warning(f'{self.device_name} - {self.Log_Pfad_conf_1[self.sprache]} gamepad_Button - {self.Log_Pfad_conf_2[self.sprache]} [HubS, HubT] - {self.Log_Pfad_conf_5_4[self.sprache].replace("; ","")} - {self.Log_Pfad_conf_8[self.sprache]} {self.Button_Link}')
            self.Button_Link = 'HubS'
            self.gamepad_aktiv = False
        ### Init:
        ...
        ## Status-N2: ##############################################################################################################################################################################################################################################################################          
        self.Stat_N2_1_Bit0     = self.sta_Bit4_str
        self.Stat_N2_1_Bit1     = self.sta_Bit11_str
        ...
        self.Stat_N2_1_Bit8     = ['Rampe ein',                                                                                                 'Ramp on']
        self.Stat_N2_1_Bit9     = ['Rampe aus',                                                                                                 'Ramp off']
        self.Stat_N2_1_Bit10    = ['', ''] # Reserve    
        self.Stat_N2_1_Bit11    = ['z-Regelung ein',                                                                                            'z-control on']
        self.Stat_N2_1_Bit12    = ['HE oben erreicht',                                                                                          'HE reached above']
        self.Stat_N2_1_Bit13    = ['HE unten erreicht',                                                                                         'HE position reached below']
        self.Stat_N2_1_Bit14    = ['TE oben erreicht',                                                                                          'TE reached above']
        self.Stat_N2_1_Bit15    = ['TE unten erreicht',                                                                                         'TE reached below']
        self.Stat_N2_2_Bit0     = ['TE oben aktiv',                                                                                             'TE top active']
        self.Stat_N2_2_Bit1     = ['TE unten aktiv',                                                                                            'TE bottom active']
        self.Stat_N2_2_Bit2     = ['SE oben erreicht',                                                                                          'SE top reached']
        self.Stat_N2_2_Bit3     = ['SE unten erreicht',                                                                                         'SE bottom reached']
        self.Stat_N2_2_Bit4     = ['SE oben aktiv',                                                                                             'SE top active']                                                                              
        self.Stat_N2_2_Bit5     = ['SE unten aktiv',                                                                                            'SE bottom active']
        self.Stat_N2_2_Bit6     = ['Syncro-Modus aktiv',                                                                                        'Syncro mode active']
        self.Stat_N2_2_Bit7     = ['Synchro aufwärts',                                                                                          'Syncro upward']
        self.Stat_N2_2_Bit8     = ['Synchro abwärts',                                                                                           'Syncro downward']
        ...
        self.Stat_Eil_N2_2_Bit4  = ['EE oben erreicht',                                                                                         'EE top reached']
        self.Stat_Eil_N2_2_Bit5  = ['EE unten erreicht',                                                                                        'EE bottom reached']
        self.Stat_Eil_N2_2_Bit6  = ['EE oben aktiv',                                                                                            'EE top active']
        self.Stat_Eil_N2_2_Bit7  = ['EE unten aktiv',                                                                                           'EE bottom active']
        ...
        kurv_dict = {                                                                   # Wert: [Achse, Farbe/Stift, Name]
            ...
            'SWxPID':   ['a1', pg.mkPen(self.color[6], width=2, style=Qt.DashDotLine),      f'{PID_Label_Soll} ({Kurven_Device_M[self.sprache]} {nemoAchse}) - {x_einzel_str[self.sprache]}<sub>{PID_Export_Soll}{sollwert_str[self.sprache]}</sub>'], 
            'IWxPID':   ['a1', pg.mkPen(self.color[7], width=2, style=Qt.DashDotLine),      f'{PID_Label_Ist} ({Kurven_Device_M[self.sprache]} {nemoAchse}) - {x_einzel_str[self.sprache]}<sub>{PID_Export_Ist}{istwert_str[self.sprache]}</sub>'],
            ...
            'oGPID':    ['a1', pg.mkPen(color=self.color[7], style=Qt.DashLine),            f'{nemoAchse} - {self.PID_G_Kurve[self.sprache]}<sub>{ober_Grenze_str[self.sprache]}</sub>'],
            'uGPID':    ['a1', pg.mkPen(color=self.color[7], style=Qt.DashDotDotLine),      f'{nemoAchse} - {self.PID_G_Kurve[self.sprache]}<sub>{unter_Grenze_str[self.sprache]}</sub>'],
        }
        ...
        # Dictionarys:
        #---------------------------------------
        self.curveDict      = {'IWs': '', 'IWv': '', 'SWv': '', 'SWs': '', 'oGv': '', 'uGv': '', 'oGs': '', 'uGs':'', 'Rezv': '', 'IWsd': '', 'SWxPID':'', 'IWxPID':'', 'Rezx': '', 'oGPID':'', 'uGPID':''}                                                                                                                                                                                 # Kurven
        for kurve in self.kurven_dict:
