...
class EducrysHeizerWidget(QWidget):
    def __init__(self, sprache, Frame_Anzeige, widget, line_color, config, config_dat, neustart, multilog_aktiv, add_Ablauf_function, eduHeizer = 'Educrys-Heizer', typ = 'Generator', parent=None):
        ...
        kurv_dict = {                                                                   # Wert: [Achse, Farbe/Stift, Name]
            ...
            'SWTPID': ['a1', pg.mkPen(self.color[5], width=2, style=Qt.DashDotLine),     f'{PID_Label_Soll} ({Kurven_Device_M[self.sprache]} {eduHeizer}) - {T_einzel_str[self.sprache]}_{cb_PID[self.sprache]}<sub>{PID_Export_Soll}{sollwert_str[self.sprache]}</sub>'], 
            'IWTPID': ['a1', pg.mkPen(self.color[6], width=2, style=Qt.DashDotLine),     f'{PID_Label_Ist} ({Kurven_Device_M[self.sprache]} {eduHeizer}) - {T_einzel_str[self.sprache]}_{cb_PID[self.sprache]}<sub>{PID_Export_Ist}{istwert_str[self.sprache]}</sub>'],
