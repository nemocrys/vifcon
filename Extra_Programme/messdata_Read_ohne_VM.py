    ...
    def update(self, label_titel, data, plot, legende):
        ...
        for n in data:
            ...
            ## y-Achsen: Label, Legende und Kurve:
                ...
                if achse_pos == 'links' and not label in label_left:        label_left += label
                elif achse_pos == 'rechts'and not label in label_right:     label_right += label
                ### Werte auslesen:
                y_str = data[n][0]
                y = []
                for wert in y_str:
                ...
                ### Legende:
                widget, side_checkbox = self.GUI_Legend_Side([n, f'{size_Fo}<sub>{extra_label}</sub>'], pen_kurve, achse)
        ...
        self.y_Label_Left  = label_left[0:-2]
        self.y_Label_Right = label_right[0:-2]
...
class GUI(QMainWindow, Cursor):
    def __init__(self, sprache, messdata_files):
        super().__init__()
        ...
        for file in self.messdata_files:
            ...
            self.knopf_Pfad.update({knopf:f'./Messdaten/{file}'})
        ...
        row = 0
        self.Bereich_Unten_Layout.addWidget(skal_Fak,                        row,          0)
        self.Bereich_Unten_Layout.addWidget(QLabel('Inhalt Datei:'),         row+1,        0)
        self.Bereich_Unten_Layout.addWidget(QLabel('Skal. Faktoren:'),       row+2,        0)
        ...
        for n in size_list:
            ...
            ### Befestigen:
            self.Bereich_Unten_Layout.addWidget(size,           row+1,          colom)
            self.Bereich_Unten_Layout.addWidget(LE,             row+2,          colom)
            if not n == size_list[-1]:
                self.Leerspalte(colom+1, self.Bereich_Unten_Layout, row+1, endRow=2)
                colom += 2
    ...
    def Kurven_Anzeige(self, state):
        ...
        self.saveName = f'{file.replace(".csv", "").replace(" ", "_").replace("./Messdaten/","")}_{date}'
        ...
        self.plot.update(f"{file.replace('.csv', '').replace('./Messdaten/','')} - {date}", werte_dict, self.plot, self.legend_achsen_Rechts_widget)
...
if __name__ == "__main__":
    ...
    mainWindow = GUI(sprache, messdata)
    app.quit
    sys.exit(app.exec_())
